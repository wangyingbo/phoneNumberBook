...
//                imageView.image = [UIImage imageNamed:array[array.count - 1]]
                _isWebImage==YES?[imageView setImageWithURL:[NSURL URLWithString:array[array.count - 1]]]:(imageView.image = [UIImage imageNamed:array[array.count - 1]]);
            }else if(i == array.count + 1){
                //最后一个imageview放第一张
//                imageView.image = [UIImage imageNamed:array[0]];
                _isWebImage==YES?[imageView setImageWithURL:[NSURL URLWithString:array[0]]]:(imageView.image = [UIImage imageNamed:array[0]]);
            }else{
                //4，1，2，3，4，1类似
//                imageView.image = [UIImage imageNamed:array[i - 1]];
                _isWebImage==YES?[imageView setImageWithURL:[NSURL URLWithString:array[i - 1]]]:(imageView.image = [UIImage imageNamed:array[i - 1]]);
            }
...
//            imageView.image = [UIImage imageNamed:array[i]];
            _isWebImage==YES?[imageView setImageWithURL:[NSURL URLWithString:array[i]]]:(imageView.image = [UIImage imageNamed:array[i]]);
...
-(void)setUpPageControl:(NSArray *)array
{
    ...
    pageControl.pageIndicatorTintColor = UIColorFromOXRGB(0xffe6eb);
    pageControl.currentPageIndicatorTintColor = UIColorFromOXRGB(0xfff9af);
...
{
    NSLog(@"点击了第%ld页",(long)self.pageControl.currentPage);
